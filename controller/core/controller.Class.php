...
class Connect extends PDO{
    public function __construct(){
        parent::__construct("mysql:host=localhost;dbname=ql_shophoa", 'root', '',
...
class Controller {
    ...
    function printData($id){
        $db = new Connect;
        $user = $db -> prepare("SELECT * FROM users where id = '$id' ORDER BY id");
        $user -> execute();
        $content = '
        <table class="table">
    ...
    function insertData($data){
        ...
        if(!$info["id"]){
            ...
            if($insertNewUser){                 
                ...
                setcookie("user_email", $data['email'], time()+3600, "/");     
            ...
            setcookie("user_email", $info['email'], time()+3600, "/");
            header('Location: ../index.php');
            exit();
        }
    }
